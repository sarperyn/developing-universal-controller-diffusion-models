...
def arg_parser():
    ...
    parser.add_argument("--replace_prompt", type=str, default="A painting of a lion eating a burger")
    ...
    parser.add_argument("--g_guidance", type=float, default=10.0, help="unconditional guidance scale: eps = eps(x, empty) + scale * (eps(x, cond) - eps(x, empty))")

    parser.add_argument("--t_enc", type=int, default=50, help="number of ddim sampling steps in inversion")
    parser.add_argument("--t_dec", type=int, default=50, help="number of ddim sampling steps in generation")
...
def main():
    ...
    # controller = au.AttentionReplace(prompts, t_dec, cross_replace_steps={"default_": 1., "lion": .4},self_replace_steps=0.4)
    
    with torch.no_grad(), autocast("cuda"):
        
        if opt.is_img2img:
            latents, midas_img = difu.inversion(opt, sampler, models)
            
        latents = init_latent(opt, latents).clone()
        x_generated = difu.generate(opt, sampler, models, latents=latents, midas_img=midas_img)
