...
def inversion(opt, prompt, sampler, models):
    x_next, midas_img = sampling(opt, prompt, sampler, models, is_generation=False)
    return x_next, midas_img


def generate(opt, prompt, sampler, models, latents=None, midas_img=None, controller=None):
    x_next = sampling(opt, prompt, sampler, models, is_generation=True, x_next=latents, midas_img=midas_img, controller=controller)
    return x_next

def sampling(opt, prompt, sampler, models, is_generation=True, x_next=None, midas_img=None, controller=None):
    ...
    ddim_num_steps = opt.t_gen if is_generation else opt.t_inv
    x_next = x_next.clone() if is_generation else get_first_stage_encoding(first_stage_model.encode(in_img), opt.scale_factor) 
    if type(prompt) is list:
        x_next = torch.cat([x_next] * opt.batch_size)
    prit = prompt if not type(prompt) is list else ' / '.join(prompt)
    guidance = opt.g_guidance if is_generation else opt.i_guidance
    desc = f'{"Inversion" if not is_generation else "Generation"}, prompt: "{prit}", scale: {guidance}, timesteps: {ddim_num_steps}'
    ...
    for i in tqdm(range(ddim_num_steps), desc=desc):
        index = opt.t_gen - i - 1 if is_generation else i            
